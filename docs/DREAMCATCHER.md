...
5. **Direct Node-to-Node Interconnect:** Inter-stage activation transport relies on low-latency, direct node-to-node network connections. InfiniBand/RDMA-class network adapters such as Intel E810 or NVIDIA Mellanox ConnectX are necessary to achieve expected performance. Stage-runner rings and the expert server exchange activations over TCP (`TCP_NODELAY`). The RPC backend (`ggml-rpc`) also has an optional RDMA (RoCE) transport tier, built when `libibverbs` is found (`GGML_RPC_RDMA`).
