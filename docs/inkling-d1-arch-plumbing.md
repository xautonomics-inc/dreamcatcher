...
## D1 verification (2026-09-16)

Verified with a synthetic Inkling GGUF generated by
`tools/make-inkling-test-gguf.py` (2 layers: 1 dense lead + 1 MoE, n_embd=256,
n_vocab=1024, d_rel=16, rel_extent=512, shortconv_k=4, 8 experts + 2 shared).

`llama-cli -m inkling-test.gguf -c 16` loaded all 31 KV pairs and **45 tensors**
with correct shapes on CPU — every `create_tensor` call in
`create_inkling_tensors()` (src/llama-load-tensors.cpp:1755) resolved its
expected dimensions, including the 3D expert and shared-expert banks and the
`[rel_extent, d_rel]` rel-proj. No `check_tensor_dims` failures. The KV-cache
allocation that follows fails only because the CPU backend reports 1 byte of
device memory against the 1 GiB safety margin (src/llama.cpp:4306,4431) — an
environmental limit, not a loader defect; it is absent on any real backend.
