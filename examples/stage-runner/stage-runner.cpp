...
// File selection and the blk_base remap live in common/layer-library.h so this
// runner and llama-server / llama-cli (--model-dir) enumerate a library the same
// way. assemble_layer_dir() is the stage-side spelling: same rules, same
// diagnostics, "stage:" prefix.
using asm_spec = layer_library::part;
...
static bool assemble_layer_dir(const std::string & dir, int win_a, int win_b, const std::string & parts_spec,
    ...
    return layer_library::assemble(dir, win_a, win_b, parts_spec, out, "stage");
}
// -----------------------------------------------------------------------------
