...
In a single-request pipeline, at S0-measured busy fractions every ring stage but the bottleneck is idle >90% of every token while waiting for activations to traverse the rest of the ring. Dreamcatcher puts that idle time to work for other sessions, enabling concurrent agent workloads through staggered wave execution:

1. **Independent Sequence Slots (`PipeSlot`):** The pipeline server (`stage-server.h`) maintains concurrent slots, assigning each incoming agent session its own sequence ID in the stage's shared KV cache, bounded by `per_seq_ctx = n_ctx / n_seq_max`.
...
3. **Assembly-Line Interleaving:** Once a stage completes its layer forward pass for agent session A, it dispatches activations downstream via `send_hidden()` and immediately yields the stage compute context (`handoff()`). The stage is instantly ready to ingest and compute the next wave for agent session B. Concurrently, downstream stages execute their layer windows for session A, keeping all ring nodes productively utilized across sessions.
4. **Time-Domain Arbitration via `gslot-runtime`:** When multiple pipeline stages or model processes are co-resident on shared accelerators or host CPUs, unmanaged execution suffers from concurrent allocation and spin-barrier / core contention (SPEC-016 §3.1). Without arbitration, co-tenant stages on one device interleave per kernel. The Global Slot Router (`tools/gslot/gslot/arbiter.py`, daemon in `tools/gslot/gslot/daemon.py`, with C++ client `examples/stage-runner/gslot_client.h` vendored at `tools/gslot/c/gslot_client.h`) arbitrates device access through two lease modes:
   - `quantum` (default, 250 ms): Single lease per time window for co-tenants each wanting continuous throughput on one device. However, a coarse time-slice hold would sit on the device while a pipeline stage waits for the ring, starving co-tenants while leaving the resource idle.
...
5. **Strict Fail-Open Resilience:** If `STAGE_GSLOT_SOCKET` is unset, `g_gslot.open()` returns `true` after one predictable-branch load of a bool. If the daemon becomes unreachable, times out, or restarts, the client returns permitted and backs off for a window: there is "no configuration option to make this fail closed", ensuring cluster inference never blocks or deadlocks on scheduler availability.
