...
Vulkan backend support (measured with Gemma-4 12B Q4_0, greedy, at the lane-9 checkpoint, which predates the published tree `20c308ca`): AMD RDNA4 and RDNA3 (RADV) flash-attention sweep clean and token-exact on chat-formatted prompts, a bare greedy prompt can differ from CPU by rounding (see meta#85); NVIDIA (coopmat1): CPU-exact; Intel ANV: self-consistent, not CPU-exact; AMD RDNA3.5 (8060S APU): verified for the expert-server path. Per-architecture status of the published model libraries is in `docs/HF-MODEL-CARDS.md`; the qwen4exp-on-Vulkan defect (meta#88) is fixed — NVIDIA coopmat1 verified, RDNA3 re-measure pending.
...
In a single-request pipeline, at S0-measured busy fractions every ring stage but the bottleneck is idle >90% of every token (`examples/stage-runner/gslot_client.h:27-28`) while waiting for activations to traverse the rest of the ring. Dreamcatcher puts that idle time to work for other sessions, enabling concurrent agent workloads through staggered wave execution:

1. **Independent Sequence Slots (`PipeSlot`):** The pipeline server (`stage-server.h:111`) maintains concurrent slots, assigning each incoming agent session its own sequence ID in the stage's shared KV cache, bounded by `per_seq_ctx = n_ctx / n_seq_max` (`stage-server.h:100`).
2. **Per-Stage Request Isolation:** Each physical stage processes **one agent's request wave at a time** (a prefill chunk or a decode wave, which can carry MTP draft tokens `mtp_k`) for its assigned layer window.
3. **Assembly-Line Interleaving:** Once a stage completes its layer forward pass for Agent Session $A$, it dispatches activations downstream via `send_hidden()` and immediately yields the stage compute context (`handoff()`). The stage is instantly ready to ingest and compute the next wave for Agent Session $B$. Concurrently, downstream stages execute their layer windows for Session $A$, keeping all ring nodes productively utilized across sessions without stalling behind the bottleneck stage's wait.
4. **Time-Domain Arbitration via `gslot-runtime`:** When multiple pipeline stages or model processes are co-resident on shared accelerators or host CPUs, unmanaged execution suffers from concurrent allocation and spin-barrier / core contention (SPEC-016 §3.1). Without arbitration, co-tenant stages on one device interleave per kernel. The Global Slot Router (`tools/gslot/gslot/arbiter.py`, daemon in `tools/gslot/gslot/daemon.py`, with C++ client `examples/stage-runner/gslot_client.h` vendored at `tools/gslot/c/gslot_client.h`) arbitrates device access through two lease modes:
   - `quantum` (default, 250 ms): Single lease per time window for continuous batching / co-tenants. However, a coarse time-slice hold would sit on the device while a pipeline stage waits for the ring, starving co-tenants while leaving the resource idle (`gslot_client.h:25-33`).
   - `burst` (`STAGE_GSLOT_MODE=burst`): Tailored for pipeline stages. Stages acquire an exclusive lease immediately before compute (`g_gslot.open()`) and release it the instant activations are emitted downstream (`g_gslot.handoff()`). Under `burst`, both tenants run full-width without hot-path threadpool resizing or spatial partitioning ("exclusivity in time instead of partitioning in space").
5. **Strict Fail-Open Resilience:** If `STAGE_GSLOT_SOCKET` is unset, `g_gslot.open()` returns `true` "after one predictable-branch load of a bool" (`gslot_client.h:14-15`). If the daemon becomes unreachable, times out, or restarts, the client fails open immediately: there is "no configuration option to make this fail closed" (`gslot_client.h:37-42`), ensuring cluster inference never blocks or deadlocks on scheduler availability.
