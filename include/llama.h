...
extern "C" {
    ...
    // source_blk_start/source_blk_count describe the source-model window represented
    // by the complete parts list. They must be identical in every list entry and let
    // the loader slice source-length, per-block metadata arrays to that window.
    ...
    // Metadata arrays whose length equals source_blk_count are sliced from
    // source_blk_start to the assembled block count, making the result equivalent
    // to a monolithic slice of the same window.
