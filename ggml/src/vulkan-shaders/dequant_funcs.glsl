...
// ------- ik_llama.cpp iqk quants (ik-vulkan-refresh) -------
// element mapping (QK_K=256, per CPU dequantize_row_* in ggml/src/iqk/iqk_quantize.cpp)

#if defined(DATA_A_IQ4_K)
FLOAT_TYPE iq4k_val(uint ib32, uint h, uint q, uint a_offset, uint ib) {
    const uint x = (uint(data_a[a_offset + ib].extra) >> (2*ib32 + h)) & 1;
    return kvalues_iq4k[16*x + q];
}
float iq4k_dl(uint ib32, uint h, uint a_offset, uint ib) {
    const uint sh = data_a[a_offset + ib].scales_h[ib32/2] >> (4 * (ib32 & 1));
    const uint sl = data_a[a_offset + ib].scales_l[ib32];
    return float(h == 0 ? int((sl & 0xF) | ((sh << 4) & 0x30)) - 32
                        : int((sl >>  4) | ((sh << 2) & 0x30)) - 32);
}
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const uint ib32 = iqs / 32;
    const uint h    = (iqs & 16) >> 4;
    const uint j    = iqs & 15;
    const uint iq   = 16 * ib32 + j;
    u8vec2 qs = u8vec2(data_a[a_offset + ib].qs[iq], data_a[a_offset + ib].qs[iq + 1]);
    qs = (qs >> (4*h)) & uint8_t(0xF);
    const float dl = iq4k_dl(ib32, h, a_offset, ib);
    return dl * vec2(iq4k_val(ib32, h, qs.x, a_offset, ib), iq4k_val(ib32, h, qs.y, a_offset, ib));
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const uint ib32 = iqs / 32;
    const uint h    = (iqs & 16) >> 4;
    const uint j    = iqs & 15;
    const uint iq   = 16 * ib32 + j;
    u8vec4 qs = u8vec4(data_a[a_offset + ib].qs[iq], data_a[a_offset + ib].qs[iq + 1], data_a[a_offset + ib].qs[iq + 2], data_a[a_offset + ib].qs[iq + 3]);
    qs = (qs >> (4*h)) & uint8_t(0xF);
    const float dl = iq4k_dl(ib32, h, a_offset, ib);
    return dl * vec4(iq4k_val(ib32, h, qs.x, a_offset, ib), iq4k_val(ib32, h, qs.y, a_offset, ib),
                     iq4k_val(ib32, h, qs.z, a_offset, ib), iq4k_val(ib32, h, qs.w, a_offset, ib));
}
#endif

#if defined(DATA_A_IQ5_K)
float iq5k_dl(uint ib64, uint sub, uint a_offset, uint ib) {
    const uint sh = data_a[a_offset + ib].scales_h[ib64];
    const uint sl = data_a[a_offset + ib].scales_l[2*ib64 + sub/2];
    const uint l  = (sub & 1) == 0 ? (sl & 0xF) : (sl >> 4);
    const uint hbits = (sh >> (2*sub)) & 3;
    return float(int(l | (hbits << 4)) - 32);
}
FLOAT_TYPE iq5k_val(uint ib64, uint sub, uint q, uint a_offset, uint ib) {
    const uint x = (uint(data_a[a_offset + ib].extra) >> (4*ib64 + sub)) & 1;
    return kvalues_iq5k[32*x + q];
}
uint iq5k_q(uint ib64, uint sub, uint j, uint a_offset, uint ib) {
    const uint qlo = (data_a[a_offset + ib].qs[32*ib64 + 16*(sub & 1) + j] >> (4*(sub/2))) & 0xF;
    const uint qh  = (data_a[a_offset + ib].qh[16*(sub & 1) + j] >> (2*ib64 + sub/2)) & 1;
    return qlo | (qh << 4);
}
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const uint ib64 = iqs / 64;
    const uint sub  = (iqs & 63) / 16;  // sub-block order: +0(lo), +16(lo), +0(hi), +16(hi)
    const uint j    = iqs & 15;
    const float dl  = iq5k_dl(ib64, sub, a_offset, ib);
    return dl * vec2(iq5k_val(ib64, sub, iq5k_q(ib64, sub, j,     a_offset, ib), a_offset, ib),
                     iq5k_val(ib64, sub, iq5k_q(ib64, sub, j + 1, a_offset, ib), a_offset, ib));
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const uint ib64 = iqs / 64;
    const uint sub  = (iqs & 63) / 16;
    const uint j    = iqs & 15;
    const float dl  = iq5k_dl(ib64, sub, a_offset, ib);
    return dl * vec4(iq5k_val(ib64, sub, iq5k_q(ib64, sub, j,     a_offset, ib), a_offset, ib),
                     iq5k_val(ib64, sub, iq5k_q(ib64, sub, j + 1, a_offset, ib), a_offset, ib),
                     iq5k_val(ib64, sub, iq5k_q(ib64, sub, j + 2, a_offset, ib), a_offset, ib),
                     iq5k_val(ib64, sub, iq5k_q(ib64, sub, j + 3, a_offset, ib), a_offset, ib));
}
#endif

#if defined(DATA_A_IQ6_K)
FLOAT_TYPE iq6k_val(uint ib64, uint sub, uint q, uint a_offset, uint ib) {
    const uint x = (uint(data_a[a_offset + ib].extra) >> (4*ib64 + sub)) & 1;
    return FLOAT_TYPE(iq6k_value(q, x));
}
uint iq6k_q(uint ib64, uint sub, uint j, uint a_offset, uint ib) {
    const uint shift = 4*(ib64 & 1);
    const uint qlo = (data_a[a_offset + ib].qs[32*ib64 + 16*(sub & 1) + j] >> (4*(sub/2))) & 0xF;
    const uint qhb = data_a[a_offset + ib].qh[32*(ib64/2) + 16*(sub & 1) + j] >> shift;
    const uint qh  = (sub/2) == 0 ? (qhb & 0x03) << 4 : (qhb & 0x0c) << 2;
    return qlo | qh;
}
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const uint ib64 = iqs / 64;
    const uint sub  = (iqs & 63) / 16;
    const uint j    = iqs & 15;
    const float dl  = float(data_a[a_offset + ib].scales[4*ib64 + sub]);
    return dl * vec2(iq6k_val(ib64, sub, iq6k_q(ib64, sub, j,     a_offset, ib), a_offset, ib),
                     iq6k_val(ib64, sub, iq6k_q(ib64, sub, j + 1, a_offset, ib), a_offset, ib));
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const uint ib64 = iqs / 64;
    const uint sub  = (iqs & 63) / 16;
    const uint j    = iqs & 15;
    const float dl  = float(data_a[a_offset + ib].scales[4*ib64 + sub]);
    return dl * vec4(iq6k_val(ib64, sub, iq6k_q(ib64, sub, j,     a_offset, ib), a_offset, ib),
                     iq6k_val(ib64, sub, iq6k_q(ib64, sub, j + 1, a_offset, ib), a_offset, ib),
                     iq6k_val(ib64, sub, iq6k_q(ib64, sub, j + 2, a_offset, ib), a_offset, ib),
                     iq6k_val(ib64, sub, iq6k_q(ib64, sub, j + 3, a_offset, ib), a_offset, ib));
}
#endif
...
#if defined(DATA_A_Q4_0) || defined(DATA_A_Q5_0) || defined(DATA_A_Q8_0) || defined(DATA_A_IQ1_S) || defined(DATA_A_IQ2_XXS) || defined(DATA_A_IQ2_XS) || defined(DATA_A_IQ2_S) || defined(DATA_A_IQ3_XXS) || defined(DATA_A_IQ3_S) || defined(DATA_A_IQ4_XS) || defined(DATA_A_IQ4_NL) || defined(DATA_A_IQ4_K) || defined(DATA_A_IQ5_K) || defined(DATA_A_IQ6_K)
vec2 get_dm(uint ib, uint a_offset) {
