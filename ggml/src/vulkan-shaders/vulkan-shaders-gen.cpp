...
// ik-vulkan-refresh: ik_llama.cpp iqk quants. They ride the generic mmv /
// mul_mm scalar+coopmat1 paths; no coopmat2 or integer-dot (q8_1) variants.
bool is_ik_iqk_quant(const std::string& type_name) {
    return type_name == "iq4_k" || type_name == "iq5_k" || type_name == "iq6_k";
}
...
void matmul_shaders(bool fp16, MatMulIdType matmul_id_type, bool coopmat, bool coopmat2, bool f16acc) {
    ...
    for (const auto& tname : type_names) {
        // ik-vulkan-refresh: no coopmat2 dequant callbacks for ik iqk quants
        if (coopmat2 && is_ik_iqk_quant(tname)) {
            continue;
        }
        ...
        if (!f16acc && !coopmat && !coopmat2 && (is_legacy_quant(tname) || (is_k_quant(tname) && !is_ik_iqk_quant(tname)) || tname == "mxfp4")) {
...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        // mul mat vec
        std::string data_a_key = "DATA_A_" + to_uppercase(tname);
        std::string shader = ((string_ends_with(tname, "_k") && !is_ik_iqk_quant(tname)) || string_starts_with(tname, "iq1_") || string_starts_with(tname, "iq2_") || string_starts_with(tname, "iq3_")) ? "mul_mat_vec_" + tname + ".comp" : "mul_mat_vec.comp";
...
#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
        if (is_legacy_quant(tname) || tname == "mxfp4" || (is_k_quant(tname) && !is_ik_iqk_quant(tname)) || tname == "iq1_s" || tname == "iq1_m") {
    ...
    string_to_spv("siglu_ik_f16",   "siglu_ik.comp",    {{"A_TYPE", "float16_t"},   {"D_TYPE", "float16_t"}}); // ik-port
    string_to_spv("siglu_ik_f32",   "siglu_ik.comp",    {{"A_TYPE", "float"},       {"D_TYPE", "float"}}); // ik-port
    ...
    string_to_spv("multi_add_ik_f32", "multi_add_ik.comp", {{"A_TYPE", "float"}, {"D_TYPE", "float"}}); // ik-port
    string_to_spv("mul_multi_add_ik_f32", "mul_multi_add_ik.comp", {}); // ik-port
...
void write_output_files() {
    ...
    for (const auto& tname : type_names) {
        if (btype == "q8_1" && (!is_legacy_quant(tname) && tname != "mxfp4" && !(is_k_quant(tname) && !is_ik_iqk_quant(tname)) && tname != "iq1_s" && tname != "iq1_m")) { // ik-vulkan-refresh: no q8_1 variants for ik iqk quants
            continue;
        }
