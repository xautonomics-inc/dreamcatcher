void load_a_to_shmem(const uint pos_a, const uint row, const uint col, const uint idx_m, const uint block, const uint end_k) {
            ...
            const uint ib = idx / 128;                   // 2 values per idx
            const uint e  = (idx % 128) * 2;             // element 0..254 (even)

            const uint ib32 = e / 32;
            const uint h    = (e & 16) >> 4;
            const uint iq   = 16 * ib32 + (e & 15);

            const uint sh = data_a[ib].scales_h[ib32/2] >> (4 * (ib32 & 1));
            const uint sl = data_a[ib].scales_l[ib32];
            const float dl = float(h == 0 ? int((sl & 0xF) | ((sh << 4) & 0x30)) - 32
                                          : int((sl >>  4) | ((sh << 2) & 0x30)) - 32);
            const uint x = (uint(data_a[ib].extra) >> (2*ib32 + h)) & 1;
            const uint tab = 16 * x;
            ...
            const float d = float(data_a[ib].d);
            const vec2 v = d * dl * vec2(kvalues_iq4k[tab + qs.x], kvalues_iq4k[tab + qs.y]);
            ...
            const uint sub  = (e & 63) / 16;
            const uint j    = e & 15;

            const uint shq = data_a[ib].scales_h[ib64];
            const uint slq = data_a[ib].scales_l[2*ib64 + sub/2];
            const uint l   = (sub & 1) == 0 ? (slq & 0xF) : (slq >> 4);
            const float dl = float(int(l | (((shq >> (2*sub)) & 3) << 4)) - 32);

            const uint x = (uint(data_a[ib].extra) >> (4*ib64 + sub)) & 1;
            const uint tab = 32 * x;

            const uint qs0 = (data_a[ib].qs[32*ib64 + 16*(sub & 1) + j    ] >> (4*(sub/2))) & 0xF;
            const uint qs1 = (data_a[ib].qs[32*ib64 + 16*(sub & 1) + j + 1] >> (4*(sub/2))) & 0xF;
            const uint qh0 = (data_a[ib].qh[16*(sub & 1) + j    ] >> (2*ib64 + sub/2)) & 1;
            const uint qh1 = (data_a[ib].qh[16*(sub & 1) + j + 1] >> (2*ib64 + sub/2)) & 1;

            const float d = float(data_a[ib].d);
            const vec2 v = d * dl * vec2(kvalues_iq5k[tab + (qs0 | (qh0 << 4))], kvalues_iq5k[tab + (qs1 | (qh1 << 4))]);
            ...
            const float dl = float(data_a[ib].scales[4*ib64 + sub]);
            const uint x = (uint(data_a[ib].extra) >> (4*ib64 + sub)) & 1;

            const uint shift = 4*(ib64 & 1);
            const uint qs0 = (data_a[ib].qs[32*ib64 + 16*(sub & 1) + j    ] >> (4*(sub/2))) & 0xF;
            const uint qs1 = (data_a[ib].qs[32*ib64 + 16*(sub & 1) + j + 1] >> (4*(sub/2))) & 0xF;
            const uint qhb0 = data_a[ib].qh[32*(ib64/2) + 16*(sub & 1) + j    ] >> shift;
            const uint qhb1 = data_a[ib].qh[32*(ib64/2) + 16*(sub & 1) + j + 1] >> shift;
            const uint q0 = qs0 | ((sub/2) == 0 ? (qhb0 & 0x03) << 4 : (qhb0 & 0x0c) << 2);
            const uint q1 = qs1 | ((sub/2) == 0 ? (qhb1 & 0x03) << 4 : (qhb1 & 0x0c) << 2);

            const float d = float(data_a[ib].d);
            const vec2 v = d * dl * vec2(iq6k_value(q0, x), iq6k_value(q1, x));
