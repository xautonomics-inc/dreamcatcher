...
#include "ggml-vulkan-ik-compat.h" // ik-port: fusion helpers + logging for ik ggml core
...
struct vk_device_struct {
    ...
    vk_pipeline pipeline_multi_add_ik_f32; // ik-port: ik GGML_OP_MULTI_ADD (MoE combine)
    vk_pipeline pipeline_mul_multi_add_ik_f32; // ik-port: ik GGML_OP_MUL_MULTI_ADD (weighted MoE combine)
    ...
    vk_pipeline pipeline_siglu_ik[2]; // ik-port: sigmoid(gate)*up
...
// ik-port: ik GGML_OP_MULTI_ADD push constants (sums nadd column-chunks per row)
struct vk_op_multiadd_ik_push_constants {
    uint32_t ne;
    uint32_t ne0, ne1;
    uint32_t nb0, nb01;
    uint32_t nadd;
...
// ik-port: ik GGML_OP_MUL_MULTI_ADD push constants (weighted MoE expert combine)
struct vk_op_mul_multi_add_ik_push_constants {
    uint32_t ne;
    uint32_t ne0;
    uint32_t nexp;
    uint32_t nb1;
...
struct ggml_backend_vk_context {
    ...
    // ik-port: temp buffer for FUSED_UP_GATE / MOE_FUSED_UP_GATE (two matmul intermediates)
    size_t prealloc_size_fug {};
    vk_buffer prealloc_fug;
    bool prealloc_fug_flip {};
    struct ggml_backend_vk_buffer_context * fug_wrap_ctx {};
    struct ggml_backend_buffer fug_wrap_buf {};
...
static bool ggml_vk_matmul_shmem_support(const vk_device& device, const std::vector<uint32_t>& warptile, bool mul_mat_id, ggml_type src0_type) {
    ...
    case GGML_TYPE_MXFP4:
        lut_size = 4*16;
        break;
    // ik-port: ik iqk quants (shared-memory value tables)
    ...
    case GGML_TYPE_IQ5_K:
        lut_size = 4*64;
        break;
    // (iq6_k uses an inline polynomial, no shared LUT)
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        if (device->coopmat_acc_f16_support) {
            ...
            CREATE_MM2(GGML_TYPE_IQ4_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ4_K],   matmul_iq4_k_f32,   mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ5_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ5_K],   matmul_iq5_k_f32,   mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM2(GGML_TYPE_IQ6_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ6_K],   matmul_iq6_k_f32,   mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            ...
            CREATE_MM(GGML_TYPE_IQ4_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ4_K].f32acc,   matmul_iq4_k_f32,   , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ5_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ5_K].f32acc,   matmul_iq5_k_f32,   , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
            CREATE_MM(GGML_TYPE_IQ6_K,   pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ6_K].f32acc,   matmul_iq6_k_f32,   , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
        ...
        CREATE_MM2(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ4_K],  matmul_id_subgroup_iq4_k_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id);
        CREATE_MM2(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ5_K],  matmul_id_subgroup_iq5_k_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id);
        CREATE_MM2(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ6_K],  matmul_id_subgroup_iq6_k_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id);
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID, REQSUBGROUPSIZE) \
        ...
        CREATE_MM2(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ4_K],  matmul_iq4_k_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);
        CREATE_MM2(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ5_K],  matmul_iq5_k_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);
        CREATE_MM2(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ6_K],  matmul_iq6_k_f32,  mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);
        ...
        if (device->subgroup_ballot && device->subgroup_require_full_support && subgroup_min_size_16) {
            ...
            CREATE_MM2(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ4_K],  matmul_id_subgroup_iq4_k_f32,  mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            CREATE_MM2(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ5_K],  matmul_id_subgroup_iq5_k_f32,  mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            CREATE_MM2(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ6_K],  matmul_id_subgroup_iq6_k_f32,  mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            ...
            CREATE_MM2(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ4_K],  matmul_id_iq4_k_f32,  mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
            CREATE_MM2(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ5_K],  matmul_id_iq5_k_f32,  mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
            CREATE_MM2(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ6_K],  matmul_id_iq6_k_f32,  mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
...
#define CREATE_MMQ(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        CREATE_MM(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ4_K].f32acc,  matmul_iq4_k_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);
        CREATE_MM(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ5_K].f32acc,  matmul_iq5_k_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);
        CREATE_MM(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat[GGML_TYPE_IQ6_K].f32acc,  matmul_iq6_k_f32,  , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);
        ...
        if (device->subgroup_ballot && device->subgroup_require_full_support && subgroup_min_size_16) {
            ...
            CREATE_MM(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ4_K].f32acc,  matmul_id_subgroup_iq4_k_f32,  , mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            CREATE_MM(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ5_K].f32acc,  matmul_id_subgroup_iq5_k_f32,  , mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            CREATE_MM(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ6_K].f32acc,  matmul_id_subgroup_iq6_k_f32,  , mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            ...
            CREATE_MM(GGML_TYPE_IQ4_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ4_K].f32acc,  matmul_id_iq4_k_f32,  , mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
            CREATE_MM(GGML_TYPE_IQ5_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ5_K].f32acc,  matmul_id_iq5_k_f32,  , mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
            CREATE_MM(GGML_TYPE_IQ6_K,  pipeline_dequant_mul_mat_mat_id[GGML_TYPE_IQ6_K].f32acc,  matmul_id_iq6_k_f32,  , mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
...
#undef CREATE_MM
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_IQ4_K][i],  "mul_mat_vec_iq4_k_f32_f32",  arr_dmmv_iq4_k_f32_f32_len[reduc16],  arr_dmmv_iq4_k_f32_f32_data[reduc16],  "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq, i+1}, 1, true, use_subgroups16, force_subgroup_size16);
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[w][GGML_TYPE_IQ4_K][i],  "mul_mat_vec_iq4_k_f16_f32",  arr_dmmv_iq4_k_f16_f32_len[reduc16],  arr_dmmv_iq4_k_f16_f32_data[reduc16],  "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq, i+1}, 1, true, use_subgroups16, force_subgroup_size16);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[w][GGML_TYPE_IQ5_K][i],  "mul_mat_vec_iq5_k_f16_f32",  arr_dmmv_iq5_k_f16_f32_len[reduc16],  arr_dmmv_iq5_k_f16_f32_data[reduc16],  "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq, i+1}, 1, true, use_subgroups16, force_subgroup_size16);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[w][GGML_TYPE_IQ6_K][i],  "mul_mat_vec_iq6_k_f16_f32",  arr_dmmv_iq6_k_f16_f32_len[reduc16],  arr_dmmv_iq6_k_f16_f32_data[reduc16],  "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq, i+1}, 1, true, use_subgroups16, force_subgroup_size16);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[w][GGML_TYPE_IQ4_K],  "mul_mat_vec_id_iq4_k_f32",  arr_dmmv_id_iq4_k_f32_f32_len[reduc16],  arr_dmmv_id_iq4_k_f32_f32_data[reduc16],  "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq}, 1, true, use_subgroups16, force_subgroup_size16);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[w][GGML_TYPE_IQ5_K],  "mul_mat_vec_id_iq5_k_f32",  arr_dmmv_id_iq5_k_f32_f32_len[reduc16],  arr_dmmv_id_iq5_k_f32_f32_data[reduc16],  "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq}, 1, true, use_subgroups16, force_subgroup_size16);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[w][GGML_TYPE_IQ6_K],  "mul_mat_vec_id_iq6_k_f32",  arr_dmmv_id_iq6_k_f32_f32_len[reduc16],  arr_dmmv_id_iq6_k_f32_f32_data[reduc16],  "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq}, 1, true, use_subgroups16, force_subgroup_size16);
    ...
    // ik-port: ik iqk quants (each thread dequantizes 16 elements)
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_IQ4_K],   "dequant_iq4_k",   dequant_iq4_k_len,   dequant_iq4_k_data,   "main", 2, 5 * sizeof(uint32_t), {256 * 16, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_IQ5_K],   "dequant_iq5_k",   dequant_iq5_k_len,   dequant_iq5_k_data,   "main", 2, 5 * sizeof(uint32_t), {256 * 16, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_IQ6_K],   "dequant_iq6_k",   dequant_iq6_k_len,   dequant_iq6_k_data,   "main", 2, 5 * sizeof(uint32_t), {256 * 16, 1, 1}, {}, 1);
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        device->buffer_type = {
            ...
            // ik-port: no .device member in ik's ggml_backend_buffer_type
...
static vk_pipeline ggml_vk_get_to_fp16(ggml_backend_vk_context * ctx, ggml_type type) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_IQ3_S:
        case GGML_TYPE_IQ4_XS:
        case GGML_TYPE_IQ4_K:   // ik-port
        case GGML_TYPE_IQ5_K:   // ik-port
        case GGML_TYPE_IQ6_K:   // ik-port
        case GGML_TYPE_IQ4_NL:
        case GGML_TYPE_MXFP4:
        case GGML_TYPE_NVFP4:
            break;
...
// ik-port: GGML_OP_FUSED_UP_GATE / GGML_OP_MOE_FUSED_UP_GATE
// dst = unary(gate_w @ x) * (up_w @ x), computed as two matmuls into a dedicated
// prealloc temp buffer followed by a GLU-split dispatch into the real dst.
// The matmuls are driven through the regular ggml_vk_mul_mat(_id) entry points via
// single-node fake graphs whose dst tensors point into the temp buffer.
...
static void ggml_vk_fused_up_gate(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * node) {
    const bool is_moe = node->op == GGML_OP_MOE_FUSED_UP_GATE;

    const ggml_tensor * w_up   = node->src[0];
    const ggml_tensor * w_gate = node->src[1];
    const ggml_tensor * x      = node->src[2];
    const ggml_tensor * ids    = is_moe ? node->src[3] : nullptr;

    // one aligned slot per intermediate; 2x2 slots (double-buffered across calls so
    // consecutive layers write disjoint regions and no WAR barrier is needed)
    const size_t align = ctx->device->properties.limits.minStorageBufferOffsetAlignment;
    const size_t half  = ggml_vk_align_size(ggml_nbytes(node), align);
    if (ctx->prealloc_size_fug < 4 * half) {
        ctx->prealloc_size_fug = 4 * half;
    }
    ggml_vk_preallocate_buffers(ctx, subctx); // may submit+wait+realloc (same pattern as prealloc_x/y)

    // (re)wrap the temp buffer so fake tensors can point at it
    if (ctx->fug_wrap_ctx == nullptr) {
        std::string wrap_name = "Vulkan_fug_tmp";
        ctx->fug_wrap_ctx = new ggml_backend_vk_buffer_context(ctx->device, vk_buffer(ctx->prealloc_fug), wrap_name);
        // note: fug_wrap_buf.iface stays zeroed - vulkan internals only read
        // buffer->context and buffer->buft; no buffer-iface calls are made on it
        ctx->fug_wrap_buf.buft    = &ctx->device->buffer_type;
        ctx->fug_wrap_buf.usage   = GGML_BACKEND_BUFFER_USAGE_ANY;
    }
    ctx->fug_wrap_ctx->dev_buffer = ctx->prealloc_fug;
    ctx->fug_wrap_buf.context     = ctx->fug_wrap_ctx;
    ctx->fug_wrap_buf.size        = ctx->prealloc_fug->size;

    // fake matmul dst tensors living in the temp buffer
    const size_t flip_off = ctx->prealloc_fug_flip ? 2 * half : 0;
    ctx->prealloc_fug_flip = !ctx->prealloc_fug_flip;

    ggml_tensor t_up = {};
    t_up.type = GGML_TYPE_F32;
    t_up.op   = is_moe ? GGML_OP_MUL_MAT_ID : GGML_OP_MUL_MAT;
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        t_up.ne[i] = node->ne[i];
    }
    t_up.nb[0] = sizeof(float);
    t_up.nb[1] = t_up.nb[0] * t_up.ne[0];
    t_up.nb[2] = t_up.nb[1] * t_up.ne[1];
    t_up.nb[3] = t_up.nb[2] * t_up.ne[2];
    t_up.buffer = &ctx->fug_wrap_buf;
    t_up.data   = (char *) vk_ptr_base + flip_off;
    snprintf(t_up.name, sizeof(t_up.name), "fug_up_tmp");

    ggml_tensor t_gate = t_up;
    t_gate.data = (char *) vk_ptr_base + flip_off + half;
    snprintf(t_gate.name, sizeof(t_gate.name), "fug_gate_tmp");

    t_up.src[0]   = (ggml_tensor *) w_up;
    t_up.src[1]   = (ggml_tensor *) x;
    t_gate.src[0] = (ggml_tensor *) w_gate;
    t_gate.src[1] = (ggml_tensor *) x;
    if (is_moe) {
        t_up.src[2]   = (ggml_tensor *) ids;
        t_gate.src[2] = (ggml_tensor *) ids;
    }

    ggml_tensor * up_nodes[1]   = { &t_up };
    ggml_tensor * gate_nodes[1] = { &t_gate };
    ggml_cgraph g_up = {};
    g_up.n_nodes = 1;
    g_up.nodes   = up_nodes;
    ggml_cgraph g_gate = {};
    g_gate.n_nodes = 1;
    g_gate.nodes   = gate_nodes;

    // No WAR barrier needed before the matmuls: this call writes the opposite
    // double-buffer slots from the previous call, and the previous call's RAW
    // barrier already ordered the call before that.
    if (is_moe) {
        ggml_vk_mul_mat_id(ctx, subctx, &g_up, 0);
        ggml_vk_mul_mat_id(ctx, subctx, &g_gate, 0);
    } else {
        ggml_vk_mul_mat(ctx, subctx, &g_up, 0);
        ggml_vk_mul_mat(ctx, subctx, &g_gate, 0);
    }
    // make the matmul writes visible to the GLU reads (RAW)
    ggml_vk_sync_buffers(ctx, subctx);

    // dst = unary(gate) * up; pipeline chosen via ggml_vk_get_glu_op_compat(node)
    ggml_vk_glu(ctx, subctx, &t_gate, &t_up, node);
}
...
// ik-port: map ik's GGML_OP_FUSED_MUL_UNARY(gate, up, unary) onto the mainline GLU
// split pipelines: unary(gate)*up == {swiglu,geglu,reglu}_split(gate, up).
static enum ggml_glu_op ggml_vk_get_glu_op_compat(const ggml_tensor * t) {
    if (t->op == GGML_OP_GLU) {
        return ggml_get_glu_op(t);
    }
    GGML_ASSERT(t->op == GGML_OP_FUSED_MUL_UNARY || t->op == GGML_OP_FUSED_UP_GATE || t->op == GGML_OP_MOE_FUSED_UP_GATE);
    switch ((enum ggml_unary_op) t->op_params[0]) {
        case GGML_UNARY_OP_SILU: return GGML_GLU_OP_SWIGLU;
        case GGML_UNARY_OP_GELU: return GGML_GLU_OP_GEGLU;
        case GGML_UNARY_OP_RELU: return GGML_GLU_OP_REGLU;
        default: GGML_ABORT("fused_mul_unary: unary op %d has no GLU mapping", (int) t->op_params[0]);
    }
}
...
static vk_pipeline ggml_vk_op_get_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * dst, ggml_op op) {
    ...
    case GGML_OP_UNARY:
        ...
        return nullptr;
    case GGML_OP_MULTI_ADD: // ik-port
        if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32 && dst->ne[2] == 1 && dst->ne[3] == 1) {
            return ctx->device->pipeline_multi_add_ik_f32;
        }
        return nullptr;
    case GGML_OP_FUSED_MUL_UNARY: // ik-port: mapped onto GLU split pipelines
    case GGML_OP_GLU:
        if ((src0->type != GGML_TYPE_F32 && src0->type != GGML_TYPE_F16) ||
            (dst->type != GGML_TYPE_F32 && dst->type != GGML_TYPE_F16) ||
            (src0->type != dst->type)) {
        ...
        // ik-port: FUSED_MUL_UNARY(SIGMOID) has no ggml_glu_op equivalent
        if (dst->op == GGML_OP_FUSED_MUL_UNARY && (enum ggml_unary_op) dst->op_params[0] == GGML_UNARY_OP_SIGMOID) {
            return ctx->device->pipeline_siglu_ik[dst->type == GGML_TYPE_F16];
        }
        switch (ggml_vk_get_glu_op_compat(dst)) {
...
// ik-port: GGML_OP_MUL_MULTI_ADD — weighted MoE expert combine (see shader).
// Custom dispatch (not ggml_vk_op_f32) because of the optional src2/src3 pair.
static void ggml_vk_mul_multi_add_ik(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * node) {
    const ggml_tensor * src0 = node->src[0];
    const ggml_tensor * src1 = node->src[1];
    const ggml_tensor * src2 = node->src[2]; // optional per-expert scales (f32)
    const ggml_tensor * src3 = node->src[3]; // optional expert ids (i32)
    const bool has_ids = src2 != nullptr && src3 != nullptr;

    GGML_ASSERT(src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 && node->type == GGML_TYPE_F32);

    vk_op_mul_multi_add_ik_push_constants pc = {
        (uint32_t)ggml_nelements(node),
        (uint32_t)node->ne[0],
        (uint32_t)src0->ne[1],
        (uint32_t)(node->nb[1]/sizeof(float)),
        (uint32_t)(src0->nb[1]/sizeof(float)), (uint32_t)(src0->nb[2]/sizeof(float)),
        (uint32_t)(src1->nb[1]/sizeof(float)), (uint32_t)(src1->nb[2]/sizeof(float)),
        has_ids ? (uint32_t)(src3->nb[1]/sizeof(int32_t)) : 0u,
        has_ids ? 1u : 0u,
    };

    vk_pipeline pipeline = ctx->device->pipeline_mul_multi_add_ik_f32;
    ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);

    static constexpr uint32_t max_tensors = 5;
    const ggml_tensor * tensors[max_tensors] = { src0, src1, has_ids ? src2 : src0, has_ids ? src3 : src0, node };
    ...
    for (uint32_t i = 0; i < max_tensors; ++i) {
        ...
        if (!uma) {
            ...
            buf[i] = buf_ctx->dev_buffer;
            offset[i] = vk_tensor_offset(tensors[i]) + tensors[i]->view_offs;
        }
        GGML_ASSERT(buf[i] != nullptr);
        GGML_ASSERT((offset[i] % ctx->device->properties.limits.minStorageBufferOffsetAlignment) == 0);
    }

    const uint32_t ne = pc.ne;
    std::array<uint32_t, 3> elements;
    if (ne > 262144) {
        elements = { 512, 512, CEIL_DIV(ne, 262144) };
    } else if (ne > 512) {
...
// ik-port: GGML_OP_FUSED_RMS_NORM(x, w) == rms_norm(x)*w — reuses mainline's fused
// RMS_NORM+MUL pipeline (selected via a temporary num_additional_fused_ops = 1).
static void ggml_vk_fused_rms_norm(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * node) {
    const ggml_tensor * src0 = node->src[0];
    const ggml_tensor * src1 = node->src[1];
    ggml_tensor * dst = node;
    const float * op_params = (const float *)node->op_params;
    ...
    ctx->num_additional_fused_ops = 1; // selects the rms_norm_mul pipeline variant
    ggml_vk_op_f32<vk_op_binary_push_constants>(ctx, subctx, src0, src1, nullptr, nullptr, dst, GGML_OP_RMS_NORM, std::move(bin));
    ctx->num_additional_fused_ops = 0;
}

// ik-port: GGML_OP_MULTI_ADD — sums nadd column-chunks per row of src0 (MoE combine)
static void ggml_vk_multi_add_ik(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, ggml_tensor * dst) {
    const uint32_t nadd = (uint32_t)dst->op_params[0];
    ggml_vk_op_f32<vk_op_multiadd_ik_push_constants>(ctx, subctx, src0, nullptr, nullptr, nullptr, dst, GGML_OP_MULTI_ADD,
            { (uint32_t)ggml_nelements(dst), (uint32_t)dst->ne[0], (uint32_t)dst->ne[1], (uint32_t)(dst->nb[1]/sizeof(float)), (uint32_t)(src0->nb[1]/sizeof(float)), nadd });
}
...
static void ggml_vk_preallocate_buffers(ggml_backend_vk_context * ctx, vk_context subctx) {
    ...
    if (ctx->prealloc_fug == nullptr || (ctx->prealloc_size_fug > 0 && ctx->prealloc_fug->size < ctx->prealloc_size_fug)) {
        VK_LOG_MEMORY("ggml_vk_preallocate_buffers(fug_size: " << ctx->prealloc_size_fug << ")");
        // ik-port: resize the FUSED_UP_GATE intermediate buffer
        if (ctx->prealloc_fug != nullptr) {
            ggml_vk_destroy_buffer(ctx->prealloc_fug);
        }
        ctx->prealloc_fug = ggml_vk_create_buffer_device(ctx->device, ctx->prealloc_size_fug);
    }
}
...
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int node_idx, ggml_tensor *node_begin, int node_idx_begin, bool last_node, bool almost_ready, bool submit){
    ...
    if (!ggml_vk_compat_node_is_compute(node)) { // ik-port: no GGML_TENSOR_FLAG_COMPUTE in ik
        return false;
    }
    ...
    case GGML_OP_UNARY:
        ...
        break;
    case GGML_OP_FUSED_RMS_NORM: // ik-port
        ggml_vk_fused_rms_norm(ctx, compute_ctx, node);
        break;
    case GGML_OP_FUSED_UP_GATE:     // ik-port
    case GGML_OP_MOE_FUSED_UP_GATE: // ik-port
        ggml_vk_fused_up_gate(ctx, compute_ctx, node);
        break;
    case GGML_OP_MULTI_ADD: // ik-port
        ggml_vk_multi_add_ik(ctx, compute_ctx, src0, node);
        break;
    case GGML_OP_MUL_MULTI_ADD: // ik-port
        ggml_vk_mul_multi_add_ik(ctx, compute_ctx, node);
        break;
    case GGML_OP_FUSED_MUL_UNARY: // ik-port: dispatched via GLU split pipelines
    case GGML_OP_GLU:
        if (node->op == GGML_OP_FUSED_MUL_UNARY && (enum ggml_unary_op) node->op_params[0] == GGML_UNARY_OP_SIGMOID) {
            ggml_vk_glu(ctx, compute_ctx, src0, src1, node); // ik-port: siglu pipeline
            break;
        }
        switch (ggml_vk_get_glu_op_compat(node)) {
        case GGML_GLU_OP_GEGLU:
        case GGML_GLU_OP_REGLU:
        case GGML_GLU_OP_SWIGLU:
        case GGML_GLU_OP_SWIGLU_OAI:
...
static void ggml_vk_cleanup(ggml_backend_vk_context * ctx) {
    ...
    ggml_vk_destroy_buffer(ctx->prealloc_split_k);
    ggml_vk_destroy_buffer(ctx->prealloc_add_rms_partials);
    ggml_vk_destroy_buffer(ctx->prealloc_fug); // ik-port
    if (ctx->fug_wrap_ctx) { // ik-port
        delete ctx->fug_wrap_ctx;
        ctx->fug_wrap_ctx = nullptr;
    }
    ctx->prealloc_size_fug = 0; // ik-port
    ggml_vk_destroy_buffer(ctx->sync_staging);
...
static void ggml_backend_vk_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) { // ik-port: void in ik
...
[[maybe_unused]] static void ggml_backend_vk_buffer_set_tensor_2d(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset,
...
[[maybe_unused]] static void ggml_backend_vk_buffer_get_tensor_2d(ggml_backend_buffer_t buffer, const ggml_tensor * tensor, void * data, size_t offset,
                                                 size_t size, size_t n_copies, size_t stride_tensor, size_t stride_data) {
...
static const char * ggml_backend_vk_buffer_get_name(ggml_backend_buffer_t buffer) { // ik-port: ik buffer iface has get_name
    ggml_backend_vk_buffer_context * ctx = (ggml_backend_vk_buffer_context *)buffer->context;
    return ctx->name.c_str();
}

// ik-port: ik's ggml_backend_buffer_i layout (get_name first, no *_2d members)
...
[[maybe_unused]] static void ggml_backend_vk_set_tensor_2d_async(ggml_backend_t backend, ggml_tensor * tensor, const void * data, size_t offset,
                                                size_t size, size_t n_copies, size_t stride_tensor, size_t stride_data) {
...
[[maybe_unused]] static void ggml_backend_vk_get_tensor_2d_async(ggml_backend_t backend, const ggml_tensor * tensor, void * data, size_t offset,
                                                size_t size, size_t n_copies, size_t stride_tensor, size_t stride_data) {
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    // ik-port: build the per-submission use-count map used by the fusion helpers
    ggml_vk_compat_refresh_use_counts(cgraph);
    ...
    while (last_node > 0 && (ggml_vk_is_empty(cgraph->nodes[last_node]) || !ggml_vk_compat_node_is_compute(cgraph->nodes[last_node]))) { // ik-port
        last_node -= 1;
    }
...
[[maybe_unused]] static void ggml_vk_graph_optimize(ggml_backend_t backend, struct ggml_cgraph * graph)
{
...
[[maybe_unused]] static void ggml_backend_vk_event_record(ggml_backend_t backend, ggml_backend_event_t event) {
...
[[maybe_unused]] static void ggml_backend_vk_event_wait(ggml_backend_t backend, ggml_backend_event_t event) {
...
// ik-port: ik's ggml_backend_i layout (old API). Events are disabled (signature
// mismatch vs the new per-device event API); graph_optimize has no slot in ik.
...
ggml_backend_t ggml_backend_vk_init(size_t dev_num) {
    ...
    ggml_backend_t vk_backend = new ggml_backend {
        ...
        // ik-port: no .device member in ik's ggml_backend
        /* .context = */ ctx,
    };
...
#if 0 // ik-port: new-API device wrappers (no landing zone in ik ggml-backend)
...
static bool ggml_backend_vk_device_supports_op_impl(size_t dev_num, const ggml_tensor * op) { // ik-port: device -> dev_num
    const vk_device& device = ggml_vk_get_device(dev_num);
    ...
    switch (op->op) {
        ...
        case GGML_OP_FUSED_UP_GATE:     // ik-port: 2x mul_mat(+id) + GLU decomposition
        case GGML_OP_MOE_FUSED_UP_GATE: // ik-port
            {
                switch ((enum ggml_unary_op) op->op_params[0]) {
                    case GGML_UNARY_OP_SILU:
                    case GGML_UNARY_OP_GELU:
                    case GGML_UNARY_OP_RELU:
                        break;
                ...
                if (op->type != GGML_TYPE_F32 || op->src[2]->type != GGML_TYPE_F32) {
                    return false;
                }
                const bool fug_is_moe = op->op == GGML_OP_MOE_FUSED_UP_GATE;
                // both weight matmuls must be supported; probe with fake MUL_MAT(_ID) ops
                ggml_tensor fake = {};
                fake.op   = fug_is_moe ? GGML_OP_MUL_MAT_ID : GGML_OP_MUL_MAT;
                fake.type = GGML_TYPE_F32;
                for (int i = 0; i < GGML_MAX_DIMS; ++i) {
                    fake.ne[i] = op->ne[i];
                }
                fake.nb[0] = sizeof(float);
                fake.nb[1] = fake.nb[0] * fake.ne[0];
                fake.nb[2] = fake.nb[1] * fake.ne[1];
                fake.nb[3] = fake.nb[2] * fake.ne[2];
                fake.src[0] = op->src[0];
                fake.src[1] = op->src[2];
                fake.src[2] = fug_is_moe ? op->src[3] : nullptr;
                if (!ggml_backend_vk_device_supports_op_impl(dev_num, &fake)) {
                    return false;
                }
                fake.src[0] = op->src[1];
                return ggml_backend_vk_device_supports_op_impl(dev_num, &fake);
            }
        case GGML_OP_FUSED_MUL_UNARY: // ik-port: unary ops with a GLU(-like) mapping
            switch ((enum ggml_unary_op) op->op_params[0]) {
                case GGML_UNARY_OP_SILU:
                case GGML_UNARY_OP_GELU:
                case GGML_UNARY_OP_RELU:
        ...
        case GGML_OP_MUL_MAT_ID:
            {
                ...
                // Allow dim-3 broadcast for the mat-vec path (dst->ne[1]==1): ggml_vk_mul_mat_vec_q_f16
                // broadcasts src0 over the batch via r3 = ne13/ne03. Keeps the MLA V-absorb (v_mla ne[3]=1
                // broadcast over n_tokens) on the GPU at n_tokens>1 instead of CPU fallback (which breaks
                // a head-split / tensor-parallel attention). src1 is permuted here so routing lands on the
                // broadcast-capable mat-vec kernel, not the nc/p021/mmq paths.
                if (a->ne[3] != b->ne[3] &&
                    !(op->op == GGML_OP_MUL_MAT && op->ne[1] == 1 && a->ne[3] == 1 && (b->ne[3] % a->ne[3]) == 0)) {
                    return false;
                }
        ...
        case GGML_OP_RMS_NORM:
            return true;
        case GGML_OP_FUSED_RMS_NORM: // ik-port: rms_norm(x)*w via mainline fused pipeline
            return op->src[0]->type == GGML_TYPE_F32 && op->src[1]->type == GGML_TYPE_F32 && op->type == GGML_TYPE_F32;
        case GGML_OP_MULTI_ADD: // ik-port
            return op->src[0]->type == GGML_TYPE_F32 && op->type == GGML_TYPE_F32 && op->ne[2] == 1 && op->ne[3] == 1;
        case GGML_OP_MUL_MULTI_ADD: // ik-port
            return op->src[0]->type == GGML_TYPE_F32 && op->src[1]->type == GGML_TYPE_F32 && op->type == GGML_TYPE_F32 &&
                   (op->src[2] == nullptr || (op->src[3] != nullptr && op->src[2]->type == GGML_TYPE_F32 && op->src[3]->type == GGML_TYPE_I32));
        case GGML_OP_NORM:
        case GGML_OP_GROUP_NORM:
            return ggml_is_contiguous(op->src[0]);
        case GGML_OP_L2_NORM:
...
// ik-port: backend-level wrappers for the old ggml_backend_i API
static bool ggml_backend_vk_supports_op(ggml_backend_t backend, const ggml_tensor * op) {
    ggml_backend_vk_context * ctx = (ggml_backend_vk_context *)backend->context;
    bool supported = ggml_backend_vk_device_supports_op_impl(ctx->device->idx, op);
    static const bool debug_unsupported = getenv("GGML_VK_SUPPORTS_DEBUG") != nullptr;
    if (debug_unsupported && !supported) {
        fprintf(stderr, "ggml_vulkan: UNSUPPORTED op=%s name=%s dst_type=%s ne=[%ld,%ld,%ld,%ld] src0=%s src1=%s\n",
...
#if 0 // ik-port: new-API device events / buffer_from_host_ptr (unused under ik)
...
#if 0 // ik-port: new-API device/registry interfaces (no landing zone in ik)
...
// ik-port: old-API registration (mirrors the pre-graft ik ggml-vulkan.cpp)
...
GGML_CALL int ggml_backend_vk_reg_devices() {
    ...
    return (int) vk_instance.device_indices.size();
}
...
// ik-port: no dynamic backend loading in ik
